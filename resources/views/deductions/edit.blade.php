        ...
        <div class="bg-white p-6 rounded-lg shadow">
            ...
            <form method="POST" action="{{ route('deductions.update', $deduction->id) }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {{-- Employee --}}
                    {{-- Employee (searchable, preselected) --}}
                    <div>
                        <label class="block mb-1">Employee</label>
                        @php
                            $selectedEmployee = $employees->firstWhere('id', $deduction->employee_id);
                        @endphp
                        <div x-data="{
                            employees: {{ Js::from(
                                $employees->map(
                            ...
                            selected: {{ Js::from([
                                ...
                                'name' => $selectedEmployee ? $selectedEmployee->first_name . ' ' . $selectedEmployee->last_name : null,
                            ]) }},
                            open: false,
                            get filtered() {
                                return this.search === '' ?
                        ...
                        <select name="deduction_type_id" class="w-full border px-3 py-2 rounded" required>
                            ...
                            @foreach ($deduction_types as $type)
                                <option value="{{ $type->id }}"
                                    {{ old('deduction_type_id', $deduction->deduction_type_id) == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    {{-- Deduction Amount --}}
                    <div>
                        <label class="block mb-1">Deduction Amount</label>
                        <input type="number" name="deduction_amount" step="0.01"
                            value="{{ old('deduction_amount', $deduction->deduction_amount) }}"
                    ...
                    <div>
                        ...
                        <input type="date" name="start_date" value="{{ old('start_date', $deduction->start_date) }}"
                            class="w-full border px-3 py-2 rounded" required>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="end_date" value="{{ old('end_date', $deduction->end_date) }}"
