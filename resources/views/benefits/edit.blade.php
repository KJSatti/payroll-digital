<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto py-12">
        <div class="bg-white p-6 rounded-lg shadow">
            ...
            @endif
            <form method="POST" action="{{ route('benefits.update', $benefit->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        @php
                            $selectedEmployee = $employees->firstWhere('id', $benefit->employee_id);
                        @endphp
                        <div x-data="{
                            employees: {{ Js::from(
                                $employees->map(
                    ...
                    {{-- Benefit Type (dropdown, preselected) --}}
                    <div>
                        ...
                        <select name="benefit_type_id" class="w-full border px-3 py-2 rounded" required>
                            ...
                            @foreach ($benefit_types as $type)
                                <option value="{{ $type->id }}"
                                    {{ old('benefit_type_id', $benefit->benefit_type_id) == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="enrollment_date"
                            value="{{ old('enrollment_date', \Illuminate\Support\Str::of($benefit->enrollment_date)->substr(0, 10)) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="monthly_contribution" step="0.01"
                            value="{{ old('monthly_contribution', $benefit->monthly_contribution) }}" required
