...
class BenefitType extends Model
{
    ...
    {
        $rules = [
            'name' => 'required|string|max:255|unique:benefit_types,name' . ($isUpdate ? ',' . $data['id'] : ''),
        ];

        return Validator::make($data, $rules);
    }

    public static function createBenefitType($request)
    {
        try {
    ...
    public static function updateBenefitType($request, $id)
    {
        try {
    ...
    public static function deleteBenefitType($id)
    {
        try {
            $benefitType = self::findOrFail($id);
